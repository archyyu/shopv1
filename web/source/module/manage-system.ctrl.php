...
if ($do == 'install') {
    ...
    logging_run("start install");
    
    $module_name = trim($_GPC['module_name']);
    $installed_module = module_fetch($module_name);

    $manifest = ext_module_manifest($module_name);
    logging_run("install ext_module_manifest:$manifest,empty:");
    if (!empty($manifest)) {
        logging_run("manifest is not empty");
        ...
        if (!empty($installed_module)) {
            ...
            if (empty($new_support)) {
                ...
                foreach ($new_support as $support => $value) {
                    logging_run("support:$support,value:$value");
