...
define('CLOUD_GATEWAY_URL', HTTP_X_FOR .'update.zhifun.cc/app/api.php');
define('CLOUD_GATEWAY_URL_NORMAL', HTTP_X_FOR .'update.zhifun.cc/app/api.php');
...
function cloud_download($path, $type = '') {
    ...
    $dat = cloud_request(HTTP_X_FOR .'update.zhifun.cc/app/api.php', $pars, $headers, 300);
    if (is_error($dat)) {
...
function cloud_cron_create($cron) {
    ...
    $result = cloud_request(HTTP_X_FOR .'update.zhifun.cc/app/api.php', $pars);
...
function cloud_auth_url($forward, $data = array()) {
    ...
    $auth_url = HTTP_X_FOR .'update.zhifun.cc/api/auth.php?__auth=' . $query;
    return $auth_url;
}
...
function cloud_flow_master_post($flow_master) {
    ...
    $dat = cloud_request(HTTP_X_FOR .'update.zhifun.cc/app/api.php', $pars, array() , 300);
    if (is_error($dat)) {
        return error(-1, '网络存在错误， 请稍后重试。' . $dat['message']);
    }
